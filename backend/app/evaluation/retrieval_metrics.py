"""
Retrieval-quality evaluation -- preserves notebook cell 43 exactly
(Recall@K, MRR against programmatically-derived ground truth).
"""
...
# Same 9 test queries and ground-truth filters as cells 39/43.
...
def get_relevant_ids(df_source: pd.DataFrame, filt: Dict[str, str]) -> Set[str]:
    """Identical to cell 43's `get_relevant_ids`."""
...
def evaluate_retrieval(retriever, queries: List[str], ground_truth_ids: List[Set[str]], k: int = 5) -> Dict:
    """Identical to cell 43's `evaluate_retrieval` (Recall@K + MRR)."""
...
def run_retrieval_evaluation(retriever, df_processed: pd.DataFrame, k_values: List[int] = None) -> pd.DataFrame:
    """Identical to cell 43's final loop building `retrieval_report_df` for k=1,3,5."""
