"""
Generation-quality evaluation -- preserves notebook cell 41 exactly
(ROUGE-1 / ROUGE-L / smoothed sentence-BLEU).
"""
import time
...
# Same fixed ground-truth eval set as cell 41.
EVAL_DATASET = [
    {
...
def calculate_metrics(bot_ans: str, ground_truth: str) -> Tuple[float, float, float]:
    """Identical to cell 41's `calculate_metrics`."""
...
def run_generation_evaluation(retriever, client, gemini_model: str, eval_dataset=None) -> "pd.DataFrame":
    """
    Identical loop/logic to cell 41: retrieve -> build prompt -> generate ->
    score -> collect into a DataFrame, with per-query latency.
    """
    import pandas as pd  # local import keeps module import-time light
